...
from models.DC_UNet import DC_Unet
...
class HoloNet(nn.Module):
    ...
    def forward(self, target_amp):
        ...
        if self.final_phase_only is None:
            # return amp, double_phase_amp_phase()
...
class InitialPhaseUnet(nn.Module):
    ...
    def __init__(self, num_down=8, num_features_init=32, max_features=256,
        ...
        # SA_UNet
        # net = [SA_UNet(in_channels=1),
        #        nn.Hardtanh(-math.pi, math.pi)]
        # ONET
        # net = [ONET(in_channels=1),
        #        nn.Hardtanh(-math.pi, math.pi)]
        # ENET
        # U2NETP
        net = [U2NETP(in_channels=1),
               nn.Hardtanh(-math.pi, math.pi)]
        # net = [DC_Unet(in_channels=1),
        #        nn.Hardtanh(-math.pi, math.pi)]
...
class FinalPhaseOnlyUnet(nn.Module):
    ...
    def __init__(self, num_down=8, num_features_init=32, max_features=256,
        ...
        #        nn.Hardtanh(-math.pi, math.pi)]
        # SA_UNet
        # net = [SA_UNet(in_channels=2),
        #        nn.Hardtanh(-math.pi, math.pi)]
        # ONET
        # net = [ONET(in_channels=2),
        #        nn.Hardtanh(-math.pi, math.pi)]
        # ENET
        # U2NETP
        net = [U2NETP(in_channels=2),
               nn.Hardtanh(-math.pi, math.pi)]
        # net = [DC_Unet(in_channels=2),
        #        nn.Hardtanh(-math.pi, math.pi)]
        self.net = nn.Sequential(*net)
...
class PhaseOnlySAUnet(nn.Module):
    """computes the final SLM phase given a target amplitude"""

    def __init__(self, num_down=10, num_features_init=16, norm=nn.BatchNorm2d):
        super(PhaseOnlySAUnet, self).__init__()
...
class PhaseOnlySCUnet(nn.Module):
    """computes the final SLM phase given a target amplitude"""

    def __init__(self, num_down=10, num_features_init=16, norm=nn.BatchNorm2d):
        super(PhaseOnlySCUnet, self).__init__()
...
class PhaseOnlyDCUnet(nn.Module):
    """computes the final SLM phase given a target amplitude"""

    def __init__(self, num_down=10, num_features_init=16, norm=nn.BatchNorm2d):
        super(PhaseOnlyDCUnet, self).__init__()

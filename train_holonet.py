...
def create_argument_parser():
    ...
    p.add_argument('--loss_fun', type=str, default='mse', help='Options: mse, l1, si_mse, vgg, vgg-low')
    ...
    p.add_argument('--net_name', type=str, default='UNet',
                   help='UNET/UNet_2Plus/UNet_3Plus/R2AttUNet/DC_Unet/UNext_S/U2NETP')
    return p.parse_args()
    # parse arguments


opt = create_argument_parser()
channel = opt.channel
run_id = opt.run_id
...
data_path = './datasets/div2k'  # path for training data

if opt.model_path == '':
    opt.model_path = f'./models/{chan_str}.pth'

# resolutions need to be divisible by powers of 2 for unet
if opt.purely_unet and str(opt.net_name).lower() == "unet":
...
elif str(opt.net_name).lower() == "onet":
    phase_generator = PhaseOnlySAUnet().to(device)
elif str(opt.net_name).lower() == "saunet":
    phase_generator = PhaseOnlySAUnet().to(device)
elif str(opt.net_name).lower() == "dcunet":
    phase_generator = PhaseOnlyDCUnet().to(device)
elif str(opt.net_name).lower() == "unet":
    phase_generator = PhaseOnlyUnet(num_features_init=32).to(device)
else:
...
for i in range(opt.num_epochs):
    ...
    for k, target in enumerate(pbar):
        ...
        pbar.set_description(f'Epoch {opt.num_epochs}/{i + 1}: {loss_val.item():.4f}')
        with torch.no_grad():
